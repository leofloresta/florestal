ace <- function(x,a,aj,E=0.1,p=0.05,ampl=2,prot=NULL,prop=F,rn=F,spivi=15,un=F,pt=T,save=T){
  ...
  for(i in x[,1]){
    ...
    for(j in breaks){
      ...
      maxlen <- max(sapply(table,length))
      fillchar <- NA

      data <- as.data.frame(do.call(cbind, lapply(table, function(x) c(x, rep(fillchar, maxlen - length(x) ) ))))
  ...
  if(pt==T){
    diam <- ggplot(data, aes(x=d, y=Freq)) +
      ...
      facet_grid(~Estrato,scales="free_x",space='free')+
